...
$uploadDir = '../uploads/materi/analisis_kinerja_sistem/';
$message = "";
$failed_message = "";
$deleteMessage = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        ...
        $stmt = $conn->prepare("INSERT INTO analisis_kinerja_sistem (file_name, file_path) VALUES (?, ?)");
...
if (isset($_GET['id'])) {
    ...
    // Ambil file
    $stmt = $conn->prepare("SELECT file_path FROM analisis_kinerja_sistem WHERE id = ?");
    ...
    // Hapus dari database
    $stmt = $conn->prepare("DELETE FROM analisis_kinerja_sistem WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
    ...
    // Redirect ke halaman analisis_kinerja_sistem.php setelah file dihapus
    header("Location: analisis_kinerja_sistem.php");
...
<head>
    ...
    <title>Materi Analisis Kinerja Sistem</title>
    <style type="text/css">
...
<body>
    ...
    <div class="main">
        <h2>Analisis Kinerja Sistem</h2>
        <form action="analisis_kinerja_sistem.php" method="POST" enctype="multipart/form-data">
    <div class="upload-area" id="upload-area">
        ...
        <table border="1">
            ...
            <tbody>
                ...
                $sql = "SELECT * FROM analisis_kinerja_sistem";
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                            ...
                            <td>
                                ...
                                <a href='analisis_kinerja_sistem.php?id={$row['id']}'>Delete</a>
                            </td>
                        </tr>";
                        $counter++;
                    }
