...
$uploadDir = '../uploads/materi/Bahasa_Ingg_Bisnis_1/';
$message = "";
$failed_message = "";
$deleteMessage = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        ...
        $stmt = $conn->prepare("INSERT INTO bing_bisnis1 (file_name, file_path) VALUES (?, ?)");
...
if (isset($_GET['id'])) {
    ...
    // Ambil file
    $stmt = $conn->prepare("SELECT file_path FROM bing_bisnis1 WHERE id = ?");
    ...
    // Hapus dari database
    $stmt = $conn->prepare("DELETE FROM bing_bisnis1 WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
    ...
    // Redirect ke halaman bing_bisnis1.php setelah file dihapus
    header("Location: Bahasa_Ingg_Bisnis_1.php");
...
<head>
    ...
    <title>Materi B. Ingg Bisnis 1</title>
    <style type="text/css">
...
<body>
    ...
    <div class="main">
        <h2>B. Ingg Bisnis 1</h2>
        <form action="Bahasa_Ingg_Bisnis_1.php" method="POST" enctype="multipart/form-data">
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                            ...
                            <td>
                                ...
                                <a href='Bahasa_Ingg_Bisnis_1.php?id={$row['id']}'>Delete</a>
                            </td>
                        </tr>";
                        $counter++;
                    }
